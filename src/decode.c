	...
	SINCE(R_13b1)
	{
			...
			|| !obj->handle.size || obj->handle.code)
		{
			...
			if (has_wrong_bitsize)
				obj->bitsize = 0;
			obj->tio.object->num_eed = 0;
		/*	FILE *logfile = fopen("D:\\dwg\\decode_error.log", "a");
			if (logfile) {

				fprintf(logfile, "DECODE_FAIL handle error: type=%d (%s) handle=" FORMAT_H "\n",
					obj->type, dwg_type_name(obj->type), ARGS_H(obj->handle));
				fclose(logfile);
			}*/
			// 原有的 stderr 输出可以删掉或保留，按需选择
			return error | DWG_ERR_INVALIDHANDLE;
		}
		LOG_TRACE("handle: " FORMAT_H " [H 5]\n", ARGS_H(obj->handle));
	}
