...
#  endif
	...
	if (error >= DWG_ERR_CRITICAL)
		...
		fprintf(stderr, "Non-critical error\n");

	int text_count = 0, mtext_count = 0, attrib_count = 0, attdef_count = 0;
	for (unsigned long i = 0; i < dwg.num_objects; i++) {
		Dwg_Object *obj = &dwg.object[i];
		if (!obj || !obj->tio.entity) continue;
		switch (obj->fixedtype) {
		case DWG_TYPE_TEXT: text_count++; break;
		case DWG_TYPE_MTEXT: mtext_count++; break;
		case DWG_TYPE_ATTRIB: attrib_count++; break;
		case DWG_TYPE_ATTDEF: attdef_count++; break;
		}
	}
	fprintf(stderr, "Parsed entities: TEXT=%d, MTEXT=%d, ATTRIB=%d, ATTDEF=%d\n",
		text_count, mtext_count, attrib_count, attdef_count);
